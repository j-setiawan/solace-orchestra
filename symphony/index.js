...
for (let oType of oscillatorTypes) {
    let header = document.createElement("H2");
    buttonsDiv.appendChild(header);
    header.innerHTML = oType
    for (let note in noteValues) {
        let noteButton = document.createElement("button");
        noteButton.innerHTML = note;
        noteButton.addEventListener("click", function () {
            playSound(note, oType);
        });
        buttonsDiv.appendChild(noteButton);
    }
}

function playSound(note, type) {
    let frequency = noteValues[note];
